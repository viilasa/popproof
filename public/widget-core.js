(function() {
  ...
  class SocialProofWidget {
    constructor(clientId, apiBaseUrl, options = {}) {
      ...
      this.debugIndicatorEl = null;
      const validPositions = new Set(['bottom-left', 'bottom-right', 'top-left', 'top-right']);
      this.debugIndicatorEnabled = this.options.debugIndicator === true;
      this.debugIndicatorText = this.options.debugIndicatorText || 'ProofPop Pixel';
      this.debugIndicatorPosition = validPositions.has(this.options.debugIndicatorPosition)
        ? this.options.debugIndicatorPosition
        : 'bottom-left';
      if (typeof this.options.debug === 'boolean') {
        CONFIG.DEBUG = this.options.debug;
      } else if (this.debugIndicatorEnabled) {
        CONFIG.DEBUG = true;
      }
      if (this.debugIndicatorEnabled && (typeof this.createDebugIndicator !== 'function' || typeof this.updateDebugIndicator !== 'function')) {
        console.warn('ProofPop: Debug indicator requested but not available in this build.');
        this.debugIndicatorEnabled = false;
      }
      this.pageData = {
      ...
      if (this.debugIndicatorEnabled) {
        if (typeof this.createDebugIndicator === 'function') {
          this.createDebugIndicator();
        }
        this._safeUpdateDebugIndicator('Widget initialized, verifying pixel...', 'info');
      }
    ...
    _safeUpdateDebugIndicator(message, status = 'info') {
      if (!this.debugIndicatorEnabled) return;
      if (typeof this.updateDebugIndicator === 'function') {
        this.updateDebugIndicator(message, status);
      }
    }
    ...
    async verifyPixel() {
      ...
      try {
        this._safeUpdateDebugIndicator('Verifying pixel...', 'info');
        ...
        const defaultAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdoaW9idXVibW52bGF1a2V5dXdlIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTM3NzA2MzcsImV4cCI6MjAwOTM0NjYzN30.Uh9KlRwxGgPRXBPSQTQXGdXEGUKYrWVXiQJrQBzHDQs';
        const verificationKey = this.apiKey || defaultAnonKey;
        
        // Include more detailed information in the verification ping
        const verificationData = {
          pixel_id: this.clientId,  // Move pixel_id to the request body
          url: window.location.href,  // Move url to the request body
        ...
        if (response.ok) {
          ...
          this._safeUpdateDebugIndicator(`Pixel verified at ${new Date().toLocaleTimeString()}`, 'success');
          ...
          this._safeUpdateDebugIndicator(`Pixel verification failed: HTTP ${response.status}`, 'error');
        ...
        this._safeUpdateDebugIndicator(`Pixel verification failed: ${error.message}`, 'error');
    ...
    trackEvent(eventType, eventData = {}) {
      ...
      try {
        ...
        fetch(
          ...
          if (response.ok) {
            this._safeUpdateDebugIndicator(`Tracked '${eventType}'`, 'success');
          } else {
            this._safeUpdateDebugIndicator(`Tracking '${eventType}' failed (${response.status})`, 'error');
          }
          ...
          this._safeUpdateDebugIndicator(`Tracking '${eventType}' failed: ${error.message}`, 'error');
        });
      } catch (error) {
        if (CONFIG.DEBUG) {
          console.debug(`Error tracking event '${eventType}':`, error.message);
    ...
    async fetchEvents() {
      ...
      try {
        ...
        if (data.success && data.events && Array.isArray(data.events)) {
          ...
          this._safeUpdateDebugIndicator(
            eventCount > 0 ? `Fetched ${eventCount} event${eventCount === 1 ? '' : 's'}` : 'No recent events yet',
        ...
        this._safeUpdateDebugIndicator(`Event fetch failed: ${error.message}`, 'error');
      }
    }
    ...
    destroy() {
      ...
      if (this.debugIndicatorEl && this.debugIndicatorEl.parentNode) {
        this.debugIndicatorEl.parentNode.removeChild(this.debugIndicatorEl);
      }
  ...
  function initializeWidget() {
    const script = document.currentScript || 
      Array.from(document.querySelectorAll('script')).find(s => 
        s.src && s.src.includes('widget-core.js')
      );
    ...
    const debugValue = parseBoolean(script.getAttribute('data-debug'));
    const debugIndicatorAttr = script.getAttribute('data-debug-indicator');
    let debugIndicator = false;
    if (debugIndicatorAttr !== null) {
      const parsedIndicator = parseBoolean(debugIndicatorAttr);
      debugIndicator = parsedIndicator === undefined ? true : parsedIndicator;
    }
    const debugIndicatorText = script.getAttribute('data-debug-indicator-text');
    const debugIndicatorPosition = script.getAttribute('data-debug-indicator-position');
    ...
    if (debugIndicator) {
      options.debugIndicator = true;
    }
    if (debugIndicatorText) {
      options.debugIndicatorText = debugIndicatorText;
    }
    if (debugIndicatorPosition) {
      options.debugIndicatorPosition = debugIndicatorPosition;
    }
    if (siteId) {
    ...
    // Initialize widget
    window.socialProofWidget = new SocialProofWidget(clientId, apiBaseUrl, options);
  }
